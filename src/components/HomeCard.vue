...
<template>
  <label
    for="profile-modal"
    class="card card-bordered shadow-xl w-[340px] 2xl:w-[400px] h-[420px] bg-white sm:mx-3 md:mx-0"
    @click="$emit('showModal')"
  >
    <div class="card-body flex-grow-0 overflow-hidden gap-4 p-6">
      <div class="flex flex-row gap-4 self-stretch">
        <BaseAvatar
          class="flex-none size-20 h-max"
          :src="user.picture"
        ...
        <div class="flex flex-col self-stretch justify-center gap-1">
          ...
          <span class="font-medium line-clamp-1">
            {{ user[UserAttributes.TITLE] }}</span
          >
      ...
      <div
        ...
        class="flex gap-2 w-3/4 overflow-hidden whitespace-nowrap"
      >
        <div
          v-for="tag in user[UserAttributes.TAGS]"
          :key="tag"
      ...
      <div
        v-if="user[UserAttributes.DESC_WHAT_CAN_I_HELP]"
        class="bg-[#F4F6FA] rounded-[0.25rem] gap-4 flex flex-col p-2"
      >
        <h3 class="font-bold text-lg">我可以提供哪些幫助?</h3>
        <p class="line-clamp-3 text-base leading-[140%]">
          {{ user[UserAttributes.DESC_WHAT_CAN_I_HELP] }}
        </p>
      </div>
    </div>
  </label>
