<template>
  <section class="my-20 lg:container lg:px-60 md:px-30 px-4">
    <article class="mx-auto my-12 px-4 lg:px-0 gap-9 flex flex-col">
      ...
      <div
        class="[&_*]:rounded-[6.25rem] [&_*]:w-[10rem] [&_*]:h-[3rem] [&_*]:text-black [&_*]:text-[1.25em] flex gap-[1.5rem] [&_*]:leading-7"
      >
        <router-link
          to="/auth?tab=signIn"
          class="btn bg-gradient-to-r from-[#6FD7FD] to-[#47DE78] border-none !w-[200px]"
        >
          立即登入
        </router-link>
        <router-link
          to="/auth"
          class="btn bg-white border-secondary-blue border-[2px] !w-[200px]"
        >
          快速註冊
        </router-link>
      </div>
  ...
  <div>
    <div
      class="w-screen bg-gradient-to-r from-[#C6F5D5] to-[#6fd7fd66] px-20 py-8 px-3 flex flex-col gap-6 md:py-10 md:px-4 xl:px-60 xl:py-12"
    >
      <h2 class="text-2xl font-bold text-center">三步驟幫你擴展海外人脈</h2>
      <span
        class="join flex flex-col md:flex-row [&>*]:{w-[15rem]} gap-5 self-stretch justify-center"
      >
        <div
          class="avatar p-5 gap-6 font-medium backdrop-blur-[20px] bg-white/40 rounded-[0.5rem]"
        >
          <div
            class="w-12 h-max rounded-full bg-white text-accent-blue p-[0.625rem] text-center self-center"
          >
            1
          </div>
    ...
    <label class="modal" for="profile-modal">
      <label
        class="modal-box flex flex-col sm:w-[520px] drop-shadow-[4_4px_4px_rgba(0,0,0,0.125)]"
        for=""
      >
          ...
          <div class="flex flex-col w-full gap-4 xs:gap-6">
            <div class="flex flex-col xs:flex-row gap-4 items-start">
              <BaseAvatar
                ...
                class="[&>div]:!w-[168px] justify-center self-center xs:self-start"
              />
              <div class="flex flex-col xs:gap-2">
                <h3 class="font-bold text-2xl">
                  {{ state.user.name }}
                </h3>
                <div class="flex line-clamp-2 overflow-hidden">
                  {{ state.user[UserAttributes.TITLE] }}
                </div>
                <BaseCountryWidget
                  :country-code="state.user[UserAttributes.COUNTRY_CODE]"
                />
                <div class="flex flex-row self-stretch gap-2">
                  <a
                    v-if="state.user.website"
                    :href="state.user.website"
                    class="w-[36px] h-[36px] bg-[#E5E8EE] rounded-full flex justify-center items-center"
                  >
                    <svg
            ...
            <div
              ...
              class="flex flex-wrap self-stretch content-start gap-2 line-clamp-2 overflow-hidden"
            >
              <div
                v-for="tag in state.user[UserAttributes.TAGS]"
                :key="tag"
                class="badge w-[80px] h-[28px] rounded-[8px] mb-2 mr-2 !p-[12px] text-sm text-center self-center bg-secondary border-none text-black-secondary"
              >
                {{ tag }}
              </div>
            </div>
            <div>
              ...
              <div
                v-if="state.user[UserAttributes.DESC_WHAT_CAN_I_HELP]"
                class="bg-[#F4F6FA] rounded-[0.25rem] gap-4 flex flex-col p-2"
              >
                <h3 class="font-bold text-lg">我可以提供哪些幫助?</h3>
                <p class="text-base leading-[140%]">
                  {{ state.user[UserAttributes.DESC_WHAT_CAN_I_HELP] }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div v-if="!isAuthenticated" class="">
          ...
          <div class="flex flex-col items-center gap-2 mt-8">
            ...
            <router-link
              ...
              :to="{
                path: 'auth',
                query: { redirect: `/?mentor=${state.user?.uid}` },
              }"
            >
              立即登入
            </router-link>
